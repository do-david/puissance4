...
#affichage du menu qui retourne nb_player et le palier de difficulté
def game_intro():
    res = []
    np = 0
    lvl = 0
    ...
    while intro:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                quit()
            #tous les blocks de buttons où l'utilisateur clique
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if (width*4/5)-50 <= mouse[0] <= (width*4/5)+50 and (height*3/4)-25 <= mouse[1] <= (height*3/4)+25 and startButton and playersButton:
                    ...
                    intro = False
        #afficher la ligne des adversaires après avoir cliqué sur start
        if(startButton):
            ...
            #affichage de la ligne des différentes difficultés de l'IA
            if(playersButton):
                #affichage des rectangles pour les buttons
